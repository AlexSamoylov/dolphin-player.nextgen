...
extern JNIMediaPlayerDrawer *drawer;
...
static int     g_min_audio_buf_size       = 0;
static int     g_min_video_buf_size       = 0;
static int     g_buffering_completed	  = 0;
...
#define FONT_FILE_NAME_SIZE 1023
static char g_font_fname[FONT_FILE_NAME_SIZE+1];
static int  g_rgb565;
static int  g_subtitle_font_size;

static int  g_squeue_size_min, g_squeue_size_max, g_stotal_queue_size, g_saudio_queue_size;
static int  g_nqueue_size_min, g_nqueue_size_max, g_ntotal_queue_size, g_naudio_queue_size;
...
static void rgb_video_image_display(VideoState *is)
{
	...
	if (!vp->display_rgb_text) {
		if (is->pictq_rindex == 0) {
			broov_on_buffering_listener(100);
		}
...
void set_screen_size(int width, int height){
	...
	g_aspect_ratio_w = width;
	g_aspect_ratio_h = height;
...
uint8_t* getNextDecodedFrame(int *size)
{
	...
	if (cur_stream->pictq_size == 0) {
		...
		vp = &cur_stream->pictq[cur_stream->pictq_rindex];
		if (time < vp->target_clock) {
			*size = 0;
			return NULL;
		}
		/* update current video pts */
		cur_stream->video_current_pts = vp->pts;
		cur_stream->video_current_pts_drift = cur_stream->video_current_pts - time;
		cur_stream->video_current_pos = vp->pos;
		cur_stream->video_current_pts_time = av_gettime();
		ULONG dur = (ULONG)get_master_clock(cur_stream);
		// update the value, only if a value change happens
		if (dur != g_current_duration) {
			broov_set_current_duration(dur);
		}
		//__android_log_print(ANDROID_LOG_INFO,"broov_player","numBytes %d",vp->numBytes); 
		if(vp != NULL){
			if(vp->pFrameRGB != NULL){
				*size = vp->numBytes;
				if(++cur_stream->pictq_rindex == VIDEO_PICTURE_QUEUE_SIZE) {
					cur_stream->pictq_rindex = 0;
				}
				SDL_LockMutex(cur_stream->pictq_mutex);
				cur_stream->pictq_size--;
				SDL_CondSignal(cur_stream->pictq_cond);
				SDL_UnlockMutex(cur_stream->pictq_mutex);
				return vp->pFrameRGB->data[0];
			}	
		}
	}
	// NO frame in display queue
	*size = 0;
	return NULL;
}
int native_render_video(uint8_t *pixels ,int width,int height, int numBytes)
{

	//__android_log_print(ANDROID_LOG_DEBUG,TAG,"start of native anative render");
	if(pixels == NULL || nativeWindow == NULL){
	__android_log_print(ANDROID_LOG_DEBUG,TAG,"EMPTY NATIVE WINDOW");
		return -1;
	}
	

	ANativeWindow_Buffer buffer;
	//buffer.format = WINDOW_FORMATRGB_565;
	if (ANativeWindow_lock(nativeWindow, &buffer, NULL) == 0) {
	//__android_log_print(ANDROID_LOG_INFO,TAG," height %d",ANativeWindow_getHeight(nativeWindow));		
	//__android_log_print(ANDROID_LOG_INFO,"Broov_Player"," width %d",ANativeWindow_getWidth(nativeWindow));		
	//__android_log_print(ANDROID_LOG_DEBUG,TAG,"Copying Pixels");
	  if (g_video_output_rgb_type == VIDEO_OUTPUT_RGB8888) {
		memcpy(buffer.bits, pixels,  g_screen_width*g_screen_height*4);
	  }else{
	  
		memcpy(buffer.bits, pixels,  g_screen_width*g_screen_height*2);
	  }
		//__android_log_print(ANDROID_LOG_DEBUG,TAG,"Copyied Pixels");
		    ANativeWindow_unlockAndPost(nativeWindow);
	}else{
	
	__android_log_print(ANDROID_LOG_DEBUG,TAG,"return non zero");
	}
	...
	//__android_log_print(ANDROID_LOG_DEBUG,TAG,"end of native anative render");
}
	...
	if (is->video_st) {
		...
		if (is->pictq_size == 0) {
			...
			if(0){
				...
				//renderVideo(vp);
				if(g_buffering_completed == 0){
					g_buffering_completed = 1;
					broov_on_buffering_listener(100);
				}
				native_render_video(vp->pFrameRGB->data[0],vp->width,vp->height,vp->numBytes);	
			}
...
static void alloc_rgb_picture(void *userdata) 
{
	...
	if (vp->pFrameRGB) {
		...
		if (vp->pFrameRGB) {
			av_freep(vp->pFrameRGB); //recommand to use av_freep 
			//av_free(vp->pFrameRGB); 
			//vp->pFrameRGB=0;
		}
		if (vp->buffer) {
			av_freep(vp->buffer); //recommand to use av_freep
			//av_free(vp->buffer); 
			//vp->buffer=0;
		}
		if (vp->display_rgb_text) {
...
static int rgb_queue_picture(VideoState *is, AVFrame *pFrame, double pts, int64_t pos) 
{
	...
	if (vp->pFrameRGB) {
#if 0
	if(vp->pFrameRGB->data[1] == NULL){
		__android_log_print(ANDROID_LOG_INFO,TAG,"data[1] == NULL");
		vp->pFrameRGB->data[1] = (uint8_t*)malloc(pFrame->linesize[1]*vp->height*sizeof(uint8_t));
		vp->pFrameRGB->data[2] = (uint8_t*)malloc(pFrame->linesize[2]*vp->height*sizeof(uint8_t));
	}
	memcpy(vp->pFrameRGB->data[0],pFrame->data[0],pFrame->linesize[0]*vp->height);
        memcpy(vp->pFrameRGB->data[1],pFrame->data[1],pFrame->linesize[1]*vp->height/2);
	memcpy(vp->pFrameRGB->data[2],pFrame->data[2],pFrame->linesize[2]*vp->height/2);

#endif
#if 0
	vp->pFrameRGB->data[0] = pFrame->data[0];
	vp->pFrameRGB->data[1] = pFrame->data[1];
	vp->pFrameRGB->data[2] = pFrame->data[2];
	if(drawer != NULL){	
		drawer->ConvertYUV2RGB(vp->width, vp->height, vp->pFrameRGB->data);
	}
#endif
#if 1	
	if (g_asm_yuv2rgb) {
...
{
	if (g_video_frame) {
		av_free(g_video_frame); // from Pablo Navarro	
		//avcodec_free_frame(g_video_frame);
		//g_video_frame = NULL;
	}
}
...
#else
		channels = av_get_channel_layout_nb_channels(wanted_channel_layout);
		freq = avctx->sample_rate;
...
#else
		is->audio_hw_buf_size = audio_open(freq, channels, 16, (SDL_AUDIO_BUFFER_SIZE));
		is->audio_src_fmt = is->audio_tgt_fmt = AV_SAMPLE_FMT_S16;
		is->audio_src_freq = is->audio_tgt_freq = freq;
		is->audio_src_channel_layout = is->audio_tgt_channel_layout = wanted_channel_layout;
		is->audio_src_channels = is->audio_tgt_channels = channels;
#endif
	}
...
#else
			is->audio_diff_threshold = 2.0 * SDL_AUDIO_BUFFER_SIZE / freq;
			memset(&is->audio_pkt, 0, sizeof(is->audio_pkt));
			packet_queue_init(&is->audioq);
#endif
			break;
...
static void stream_component_close(VideoState *is, int stream_index)
{
	...
	switch(avctx->codec_type) {
		case AVMEDIA_TYPE_AUDIO:
			...
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Wait for Audio Thread");
			SDL_WaitThread(is->audio_tid, NULL);
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Wait for Audio Thread Over");
...
__android_log_print(ANDROID_LOG_DEBUG,"BroovPlayer","after update aspect ratio");
//#ifdef BROOV_USE_BITMAP
	__android_log_print(ANDROID_LOG_DEBUG,"broov_player","source_width_height %d",g_source_width_height);
	if (cur_stream->video_st) {
		__android_log_print(ANDROID_LOG_DEBUG,"BroovPlayer","calling native draw");
		//drawer->loadTextures(cur_stream->video_st->codec->width,cur_stream->video_st->codec->height);	
		
		//drawer->updateVertices(cur_stream->video_st->codec->width,cur_stream->video_st->codec->height);
		set_width_height(cur_stream->video_st->codec->width, cur_stream->video_st->codec->height);
	}else{
		__android_log_print(ANDROID_LOG_DEBUG,"BroovPlayer","calling native draw");
		//drawer->loadTextures(g_aspect_ratio_w,g_aspect_ratio_h);
		//drawer->updateVertices(g_aspect_ratio_w,g_aspect_ratio_h);
		set_width_height(g_aspect_ratio_w, g_aspect_ratio_h);
	}
//#endif
	// sending information to upperLayer
	broov_on_prepared_listener();
	return 0;
...
int player_init()
{
	...
	/*if (!try_to_set_best_video_mode(fs_screen_width, fs_screen_height, g_rgb565)) {
		return 3;
	}
	*/

	{
		int font_init = broov_font_init(g_font_fname, g_subtitle_font_size);
		if (font_init == 0) {
...
int player_exit()
{
	...
	broov_on_completion_listener();
	delete_global_references();
	return 0;
}
	...
	for(;;) {
		...
		if (is->paused) {
			...
			continue;
                ...
                if (audio_paused==1) {
                    BroovNDK_ResumeAudio();
                    audio_paused=0;
                }


		// dont play file until the buffer reaches the limit if end of file reaches within the spefied limit we can play
		if(!is->is_eof)
		{	
			if(g_audio_file_type) // audio file 
			{
				if ((is->audioq.size!=0) && (is->audioq.size<g_min_audio_buf_size))
				{
					memset(audioBuffer, 0, audioBufferSize);
				}
				else
				{
			 		sdl_audio_callback(is, audioBuffer, audioBufferSize);
				}
				
			}
#if 0
     //is this logic required for audio thread ?? - AA
			else if ((is->videoq.size!=0) && (is->videoq.size<g_min_video_buf_size))
			{
                                // for video files, wait until min video buffer size is filled
				memset(audioBuffer, 0, audioBufferSize);
			}
#endif
			else
...
static int video_thread(void *arg)
{
	...
	for(;;) {
		...
		// dont play file until the buffer reaches the limit if end of file reaches within the spefied limit we can play
		if (is->videoq.size<g_min_video_buf_size && !is->is_eof) 
		{
			//__android_log_print(ANDROID_LOG_INFO,"BroovPlayer","video thread inside waiting");
			usleep(10000); //Sleep for 10ms		    
			continue;
		}
		...
		if (in_sync) {
				...
				//if (is->videoq.size < 100KB)
			{
				...
				//return 2;
                                ...
                                //check, if we have reached another lower bound and start reading data
                                //from file or stream
                                if (is->videoq.size < (g_min_video_buf_size + (10000))) {
					in_sync=true;
...
is->is_eof = g_eof;
		...
		if (!g_eof) {
			...
			//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "decode thread  is->g_audio_file_type %d ",g_audio_file_type);
			if(g_audio_file_type)
			{
				//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "decode thread  is->is_buffered %d ",is->is_buffered);
				if (is->audioq.size < g_min_audio_buf_size) 
				{
					float audio_percentage = ((float)is->audioq.size/(float)g_min_audio_buf_size)*100;
					if (audio_percentage >0 && audio_percentage<100) {
						broov_on_buffering_listener(audio_percentage);
						is->is_buffered = true;
					}
				}else if(is->is_buffered) {
					broov_on_buffering_listener(100);
					is->is_buffered = false;
				}

			}
			else
			{
				//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "video thread %d g_min_video_buf_size %d",is->videoq.size,g_min_video_buf_size);
				if ( ((is->videoq.size!=0) && (is->videoq.size < g_min_video_buf_size)) || 
                                     ((is->audioq.size!=0) && (is->audioq.size < g_min_audio_buf_size)) )
				{
					float video_percentage = ((float)is->videoq.size/(float)g_min_video_buf_size)*100;
					float audio_percentage = ((float)is->audioq.size/(float)g_min_audio_buf_size)*100;
					//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "percentage %f",video_percentage);

					int avg_percentage =  (video_percentage+audio_percentage)/2;
					if (is->audioq.size ==0) {
						avg_percentage = video_percentage;
					}
					if (is->videoq.size ==0) {
						avg_percentage = audio_percentage;
					}

					if(avg_percentage>0&&avg_percentage<=100) {  
						//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "percentage %f",avg_percentage);
						broov_on_buffering_listener(avg_percentage);
						is->is_buffered = true;
					}
...
static void broov_init_global_values_main(int loop_after_play, int audio_file_type, int sync_type, int seek_duration, int is_stream, int min_audio_buf,int min_video_buf)
{
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Loop:%d FileType:%d ", loop_after_play, audio_file_type);
	...
	if (is_stream == 1) {
		if (g_squeue_size_min) { BROOV_VIDEO_MIN_BUFFER_SIZE = g_squeue_size_min; }
		if (g_squeue_size_max) { BROOV_VIDEO_MAX_BUFFER_SIZE = g_squeue_size_max; }
		if (g_stotal_queue_size) { BROOV_TOTAL_MAX_BUFFER_SIZE= g_stotal_queue_size; }
		if (g_saudio_queue_size) { MAX_AUDIOQ_SIZE = g_saudio_queue_size; }
	} else {
		if (g_nqueue_size_min) { BROOV_VIDEO_MIN_BUFFER_SIZE = g_nqueue_size_min; }
		if (g_nqueue_size_max) { BROOV_VIDEO_MAX_BUFFER_SIZE = g_nqueue_size_max; }
		if (g_ntotal_queue_size) { BROOV_TOTAL_MAX_BUFFER_SIZE= g_ntotal_queue_size; }
		if (g_naudio_queue_size) { MAX_AUDIOQ_SIZE = g_naudio_queue_size; }
        }
	if(min_audio_buf){
        g_min_audio_buf_size = min_audio_buf;
    }
    if (min_video_buf) {
        g_min_video_buf_size = min_video_buf;
    }
	return;
} //broov_init_global_values_main()


static void broov_init_global_values_settings(int skip_frames, int rgb565, int yuv_rgb_asm, int skip_bidir_frames, 
		int vqueue_size_min, int vqueue_size_max, int total_queue_size, int audio_queue_size, 
		int fast_mode, int debug_mode, int ffmpeg_flags,
		int nqueue_size_min, int nqueue_size_max, int ntotal_queue_size, int naudio_queue_size)
{
	...
	if (vqueue_size_min) { g_squeue_size_min = vqueue_size_min; }
	if (vqueue_size_max) { g_squeue_size_max = vqueue_size_max; }
	if (total_queue_size) { g_stotal_queue_size = total_queue_size; }
	if (audio_queue_size) { g_saudio_queue_size = audio_queue_size; }

	if (nqueue_size_min) { g_nqueue_size_min = nqueue_size_min; }
	if (nqueue_size_max) { g_nqueue_size_max = nqueue_size_max; }
	if (ntotal_queue_size) { g_ntotal_queue_size = ntotal_queue_size; }
	if (naudio_queue_size) { g_naudio_queue_size = naudio_queue_size; }
...
int player_settings(char *font_fname, int subtitle_show, int subtitle_font_size, int subtitle_encoding_type, 
		int skip_frames, int rgb565, int yuv_rgb_asm, int skip_bidir_frames, 
		int vqueue_size_min, int vqueue_size_max, int total_queue_size, int audio_queue_size, 
		int fast_mode, int debug_mode, int ffmpeg_flags,
		int nqueue_size_min, int nqueue_size_max, int ntotal_queue_size, int naudio_queue_size)
{
     ...
     if (font_fname) {
         memset(g_font_fname, 0, FONT_FILE_NAME_SIZE+1);
         strncpy(g_font_fname, font_fname, FONT_FILE_NAME_SIZE);
     }

     g_subtitle_encoding_type = subtitle_encoding_type;
     g_subtitle_font_size = subtitle_font_size;
     ...
     broov_init_global_values_settings(skip_frames, rgb565, yuv_rgb_asm, skip_bidir_frames, 
		vqueue_size_min, vqueue_size_max, total_queue_size, audio_queue_size, 
		fast_mode, debug_mode, ffmpeg_flags,
		nqueue_size_min, nqueue_size_max, ntotal_queue_size, naudio_queue_size);
}


int player_main(int argc, char *argv[], 
		int loop_after_play, int audio_file_type, int sync_type, int seek_duration, int is_stream, int min_audio_buf, int min_video_buf)

{
	...
	int decode_scheduler = 0;
        ...
        broov_init_global_values_main(loop_after_play, audio_file_type, sync_type, seek_duration, is_stream, min_audio_buf,min_video_buf);
	...
	if (!is->parse_tid) {
		//av_free(is); // recommended to use av_free instead 
		av_freep(is);
		return NULL;
	}
	...
	if (is->audio_st) {
		...
		if (!is->audio_tid) {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "AudioThread Creation Failed");
			return NULL;			
		}
	}
...
void refreshVideo()
{
	...
	__android_log_print(ANDROID_LOG_DEBUG,"BROOV PLAYER","NATIVE VIDEO REFRESH");
	VideoPicture *vp;	
	if(cur_stream == NULL){

	__android_log_print(ANDROID_LOG_DEBUG,"BROOV PLAYER","NATIVE VIDEO REFRESH cur_stream NULL");
		return;
	}
	while (cur_stream->pictq_size<=0){
		usleep(5000);
	}
		//nothing to do, no picture to display in the queue
...
//-----------------------------------------------------------------------------
void renderVideo(VideoPicture *vp)
{	
	if(drawer != NULL ){
	    __android_log_print(ANDROID_LOG_DEBUG,"BroovPlayer"," vp %p is->drawer %p",cur_stream,drawer);
	    drawer->draw(vp->width, vp->height);
	}
}
